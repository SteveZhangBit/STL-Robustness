...
os.makedirs('old_gifs/car-run-ppo', exist_ok=True)
...
data1 = experiment.run_diff_max_samples('CMA', samples, out_dir='old_data/car-run-ppo/cma')
...
plt.savefig('old_gifs/car-run-ppo/fig-robustness.png', bbox_inches='tight')

# Use Random Solver
random_solver = RandomSolver(sys_eval)
evaluator2 = Evaluator(prob, random_solver)
experiment2 = Experiment(evaluator2)
data2 = experiment2.run_diff_max_samples('Random', samples, out_dir='old_data/car-run-ppo/random')
...
evaluator = Evaluator(prob, solver)
experiment = Experiment(evaluator)
data3 = experiment.run_diff_max_samples('STL2', samples, out_dir='old_data/car-run-ppo/stl2')
idx = np.argmin(data3['min_dist'])
plt.figure()
evaluator.heatmap(
    speed, steering, 25, 25,
    x_name="Speed Multiplier", y_name="Steering Multiplier", z_name="System Evaluation $\Gamma$",
    out_dir='old_data/car-run-ppo/stl2',
...
plt.savefig('old_gifs/car-run-ppo/fig-robustness-stl2.png', bbox_inches='tight')
...
plt.savefig('old_gifs/car-run-ppo/fig-boxplot.png', bbox_inches='tight')
...
# data3, _ = experiment3.run_diff_max_samples('Expc', np.arange(25, 126, 25), out_dir='old_data/car-run-ppo/expc')
# plt.figure()
# evaluator3.heatmap(
#     speed, steering, 25, 25,
#     x_name="Speed Multiplier", y_name="Steering Multiplier", z_name="System Evaluation $\Gamma$",
#     out_dir='old_data/car-run-ppo/expc',
#     boundary=np.min(data3),
# )
# plt.title('Robustness $\hat{\Delta}: ||\delta - \delta_0||_2 < %.3f$' % np.min(data3))
# plt.savefig('old_gifs/car-run-ppo/fig-robustness-expc.png', bbox_inches='tight')
...
#         ['CMA', 'CMA-Expc'])
# plt.savefig('old_gifs/car-run-ppo/fig-boxplot-expc.png', bbox_inches='tight')
