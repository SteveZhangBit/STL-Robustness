'''
The run script for the WTK example with the traditional agent.
'''
...
radius = evaluator.smooth_boundary(0.1, 1000, 0.05, 0.9, 'data/WTK/traditional')
# radius = 0.0
...
evaluator.heatmap(
    ...
    out_dir='data/WTK/traditional',
    boundary=radius,
)
plt.title('Smooth Robustness $\hat{\Delta}: ||\delta - \delta_0||_2 < %.3f$' % radius)
plt.savefig('gifs/WTK/traditional/fig-smooth-robustness.png', bbox_inches='tight')
