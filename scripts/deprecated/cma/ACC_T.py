...
data1 = experiment.run_diff_max_samples('CMA', samples, out_dir='data/ACC/traditional/cma')
...
plt.savefig('gifs/ACC/traditional/fig-robustness.png', bbox_inches='tight')

# Use Random Solver
random_solver = RandomSolver(sys_eval)
evaluator2 = Evaluator(prob, random_solver)
experiment2 = Experiment(evaluator2)
data2 = experiment2.run_diff_max_samples('Random', samples, out_dir='data/ACC/traditional/random')
...
boxplot(
    [
        [data1['min_dist'].loc[x].clip(0.0, 1.0) for x in samples],
        [data2['min_dist'].loc[x].clip(0.0, 1.0) for x in samples],
    ],
    ['red', 'blue'],
    samples * (1 + solver.options()['restarts']),
    ['CMA', 'Random'],
)
plt.savefig('gifs/ACC/traditional/fig-boxplot.png', bbox_inches='tight')
