'''
This is only for generating the figures, the data is generated from another script.
'''
import os
...
records_cma = experiment.record_min_violations(out_dir='data/cartpole-dqn/cma_heuristic')
records_cma, violations_cma = experiment.summarize_violations(records_cma, 'data/cartpole-dqn/cma_heuristic')
# Plot all samples
for i in range(len(records_cma)):
    ...
    plt.savefig(f'gifs/cartpole-dqn/fig-violations-cma-heuristic-{i}.png', bbox_inches='tight')
...
plt.savefig(f'gifs/cartpole-dqn/fig-violations-cma-heuristic-all.png', bbox_inches='tight')
