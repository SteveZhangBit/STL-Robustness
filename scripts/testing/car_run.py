...
os.makedirs('gifs/car-run-ppo-heuristic', exist_ok=True)
...
# # added by parv to save the traj first
...
# records_cma = experiment.record_min_violations(out_dir='data/car-run-ppo-heuristic/cma')
# records_cma, violations_cma = experiment.summarize_violations(records_cma, 'data/car-run-ppo-heuristic/cma')
# # Plot all samples
# for i in range(len(records_cma)):
#     samples = [(X, Y) for (X, Y, _) in records_cma[i]]
#     experiment.plot_samples(samples, 'Speed', 'Steering', 'data/car-run-ppo-heuristic', n=20)
#     plt.title('Violations found by CMA')
#     plt.savefig(f'gifs/car-run-ppo-heuristic/fig-violations-cma-{i}.png', bbox_inches='tight')

# experiment.plot_samples([[(X, Y) for (X, Y, _) in r] for r in records_cma], 'Speed', 'Steering', 'data/car-run-ppo-heuristic', n=20)
# # plt.title('Car-Run-PPO with CMA')
# plt.savefig(f'gifs/car-run-ppo-heuristic/fig-violations-cma-all.png', bbox_inches='tight')
