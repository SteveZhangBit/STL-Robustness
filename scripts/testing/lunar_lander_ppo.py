...
os.makedirs('gifs/lunar-lander-ppo-heuristic', exist_ok=True)
...
agent = PPO('/usr0/home/parvk/cj_project/STL-Robustness/models/lunar-lander/ppo')
phi = SafetyProp()
...
# # parv's addition to save delta 0 trace basically with save_best set to true for the delta 0 file path (check the cma sys_eval)
# v, x0 = sys_eval.eval_sys(env.delta_0, prob, save_best=False)
...
records_cma = experiment.record_min_violations(out_dir='data/lunar-lander-ppo-heuristic/cma')
records_cma, violations_cma = experiment.summarize_violations(records_cma, 'data/lunar-lander-ppo-heuristic/cma')
# Plot all samples
for i in range(len(records_cma)):
    ...
    plt.savefig(f'gifs/lunar-lander-ppo-heuristic/fig-violations-cma-{i}', bbox_inches='tight')
...
plt.savefig(f'gifs/lunar-lander-ppo-heuristic/fig-violations-cma-all', bbox_inches='tight')
