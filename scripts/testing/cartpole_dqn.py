...
os.makedirs('gifs/cartpole-dqn-heuristic', exist_ok=True)
...
agent = DQN('/usr0/home/parvk/cj_project/STL-Robustness/models/cartpole/best_dqn.zip')
phi = SafetyProp()
...
# added by parv for nominal trace measurement
...
records_cma = experiment.record_min_violations(out_dir='data/cartpole-dqn-heuristic/cma')
records_cma, violations_cma = experiment.summarize_violations(records_cma, 'data/cartpole-dqn-heuristic/cma')
# Plot all samples
for i in range(len(records_cma)):
    samples = [(X, Y) for (X, Y, _) in records_cma[i]]
    experiment.plot_samples(samples,  'Mass', 'Force', 'data/cartpole-dqn-heuristic', n=20)
    plt.title('Violations found by CMA')
    plt.savefig(f'gifs/cartpole-dqn-heuristic/fig-violations-cma-{i}.png', bbox_inches='tight')

experiment.plot_samples([[(X, Y) for (X, Y, _) in r] for r in records_cma],  'Mass', 'Force', 'data/cartpole-dqn-heuristic', n=20)
# plt.title('Cart-Pole-DQN with CMA')
plt.savefig(f'gifs/cartpole-dqn-heuristic/fig-violations-cma-all.png', bbox_inches='tight')
