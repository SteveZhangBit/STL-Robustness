...
records_cma = experiment.record_min_violations(out_dir='data/LKA/RL/cma')
records_cma, violations_cma = experiment.summarize_violations(records_cma, 'data/LKA/RL/cma')
# Plot all samples
for i in range(len(records_cma)):
    samples = [(X, Y) for (X, Y, _) in records_cma[i]]
    experiment.plot_samples(samples, 'Turn 1', 'Turn 2', 'data/LKA/RL', n=20)
    plt.title('Violations found by CMA')
    plt.savefig(f'gifs/LKA/RL/fig-violations-cma-{i}.png', bbox_inches='tight')
...
if min_violation is not None:
    radius = evaluator.unsafe_region(min_violation, 0.1, 0.05, 'data/LKA/RL', n=1000)
    experiment.plot_unsafe_region(min_violation, radius, 'Turn 1', 'Turn 2', 'data/LKA/RL', n=20)
    plt.title('Unsafe region found by CMA')
    plt.savefig('gifs/LKA/RL/fig-unsafe-region-cma.png', bbox_inches='tight')
...
records_random = experiment.record_min_violations(out_dir='data/LKA/RL/random')
records_random, violations_random = experiment.summarize_violations(records_random, 'data/LKA/RL/random')
# Plot all samples
for i in range(len(records_random)):
    samples = [(X, Y) for (X, Y, _) in records_random[i]]
    experiment.plot_samples(samples, 'Turn 1', 'Turn 2', 'data/LKA/RL', n=20)
    plt.title('Violations found by random search')
    plt.savefig(f'gifs/LKA/RL/fig-violations-random-{i}.png', bbox_inches='tight')
...
if min_violation is not None:
    ...
    plt.savefig('gifs/LKA/RL/fig-unsafe-region-random.png', bbox_inches='tight')
