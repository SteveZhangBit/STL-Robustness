...
from robustness.envs.cartpole import DevCartPole2, SafetyProp, SafetyProp2
...
def cartmodel(static: Sequence[float], times: SignalTimes, signals: SignalValues) -> CartpoleDataT:
   ...
   delta = normalize(static[0:4], env.dev_bounds)
   delta_0 = normalize(env.delta_0, env.dev_bounds)
   dist = np.sqrt( np.sum((delta - delta_0) ** 2) )
   env, x0bounds = env.instantiate(static[0:4])
   # set the initial state after
   obs = env.reset_to(static[4:8])  
   simulation_state = []
...
def create_new_rob_csv(filename):
        ...
        for index,row in data.iterrows():
            ...
            e, x0bounds = env.instantiate([row['d1'], row['d2'], row['d3'], row['d4']])
            ...
            if score < 0:
                final_deltas.append([row['d1'],row['d2'],row['d3'],row['d4'], score])
            
        with open('baseline_results/delta_cartpole_4.csv', mode='w', newline='') as file:
...
if __name__ == "__main__":
    filename = "baseline_results/cartpole_data_4.csv"
    if not os.path.isfile(filename):
        ...
        options = Options(runs=300, iterations=300, interval=(0, 1), static_parameters=[(0.1,2.0),(0.05,0.15), (0.25,0.75),(1.0,20.0),(-0.05,0.05),(-0.05,0.05),(-0.05,0.05), (-0.05,0.05)])
        result = staliro(cartmodel, specification, optimizer, options)
        import csv 

        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['d1','d2','d3','d4', 'i1', 'i2','i3', 'i4', 'Cost'])
            for run in result.runs:
                evaluation = worst_eval(run)
                writer.writerow([evaluation.sample[0],evaluation.sample[1], evaluation.sample[2],evaluation.sample[3],evaluation.sample[4],evaluation.sample[5],evaluation.sample[6],evaluation.sample[7], evaluation.cost])
    else:
