...
from robustness.envs.car_run import DevCarRun2, SafetyProp, SafetyProp2
...
def car_run_model(static: Sequence[float], times: SignalTimes, signals: SignalValues) -> CarRunDataT:
   ...
   delta = normalize(static[0:3], env.dev_bounds)
   ...
   env, x0bounds = env.instantiate(static[0:3])
   # set the initial state after
   obs = env.reset_to(static[3:6])  
   simulation_state = []
...
def create_new_rob_csv(filename):
        ...
        for index,row in data.iterrows():
            # set the deviation params first (steering and speed)
            e, x0bounds = env.instantiate([row['d1'], row['d2'], row['d3']])
            ...
            if score < 0:
                final_deltas.append([row['d1'],row['d2'],row['d3'], score])
            
        with open('baseline_results/delta_car_run_3.csv', mode='w', newline='') as file:
...
if __name__ == "__main__":
    filename = "baseline_results/car_run_data_3.csv"
    if not os.path.isfile(filename):
        ...
        options = Options(runs=300, iterations=300, interval=(0, 1), static_parameters=[(5.0,35.0),(0.2,0.8),(5,50.0),(-0.1,0.1),(-0.1,0.1),(2.35619449, 3.92699082)])
        result = staliro(car_run_model, specification, optimizer, options)
        import csv 
        with open(filename, 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(['d1','d2','d3', 'i1', 'i2', 'i3', 'Cost'])
