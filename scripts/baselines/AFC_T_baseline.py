...
records_ol = experiment.record_min_violations(out_dir='data/AFC/traditional/ol')
records_ol, violations_ol = experiment.summarize_violations(records_ol, 'data/AFC/traditional/ol')
# Plot all samples
for i in range(len(records_ol)):
    ...
    plt.savefig(f'gifs/AFC/traditional/fig-violations-ol-{i}.png', bbox_inches='tight')

experiment.plot_samples([[(X, Y) for (X, Y, _) in r] for r in records_ol], 'MAF Sensor Tolerance', 'AF Sensor Tolerance', 'data/AFC/traditional', n=20)
# plt.title('AFC-PID with OneLayer')
plt.savefig(f'gifs/AFC/traditional/fig-violations-ol-all.png', bbox_inches='tight')
