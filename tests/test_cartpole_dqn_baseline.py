...
def plot_csv_samples(filename, experiment):
    ...
    plt.savefig(f'tests/gifs/cartpole-dqn/cartpole_dqn_d2.png', bbox_inches='tight')
...
def create_new_rob_csv(filename):
    ...
    with open('tests/data/cartpole-dqn/delta_cartpole.csv', mode='w', newline='') as file:
...
if __name__ == "__main__":
    filename = "tests/data/cartpole-dqn/cartpole_data.csv"
    if not os.path.isfile(filename):
        ...
        options = Options(runs=5, iterations=10, interval=(0, 1), static_parameters=[(0.0,2.0),(0.0,20.0),(-0.05,0.05),(-0.05,0.05),(-0.05,0.05), (-0.05,0.05)])
        ...
        # forces = [1.0, 20.0]
        # env = DevCartPole(masses, forces, (1.0, 10.0))
        # agent = DQN('models/cartpole/best_dqn')
        # phi = SafetyProp()
