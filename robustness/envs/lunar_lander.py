...
class DevLunarLander(DeviatableEnv):
    def __init__(self, winds, turbulences, delta_0):
        assert winds[0] <= delta_0[0] <= winds[1] and turbulences[0] <= delta_0[1] <= turbulences[1], \
        ...
        self.dev_bounds = np.array([winds, turbulences])
    
    def instantiate(self, delta):
        return LunarLander(enable_wind=True, continuous=True, wind_power=delta[0], turbulence_power=delta[1]), self.x0_bounds
...
class SafetyProp(STLEvaluator):
    ...
    def prop(self):
        ...
        angle_threshold = normalize(45 * 2 * np.pi / 360, self.angle_range)
    ...
    def build_signal(self, record, time_index):
        return {
            'angle': stl.Signal(
                normalize(np.abs(record[:, 4]), self.angle_range),
                time_index
            ),
            'delta_x': stl.Signal(
                normalize(np.abs(record[:, 0]), self.x_range) -\
                    normalize(np.abs(0.6 * record[:, 1]), self.y_range),
                time_index
            )
        }
