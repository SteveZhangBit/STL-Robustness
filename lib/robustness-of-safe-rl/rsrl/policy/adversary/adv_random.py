...
from rsrl.policy.solver import TorchSolver
...
class AdvGaussian(Adv):
    ...
    def attack_batch(self, policy, obs, bound: float):
        ...
        epsilon = torch.distributions.normal.Normal(
            0, bound / 3 + 1e-5).sample([batch, self.obs_dim])
        epsilon.clamp_(-bound, bound)
        return to_device(epsilon)
...
if __name__ == "__main__":
    ...
    adv_gaussian = AdvUniform(2)

    obs = np.zeros(1000)

    eps = adv_gaussian.attack_batch(None, obs, 0.005)
